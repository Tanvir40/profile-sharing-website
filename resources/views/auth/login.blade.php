...
<html lang="en">
  ...
  <body translate="no" style="background-color: rgb(210,180,140);!important">
    <div class="login-page">
      <div class="form">
        <h3>Sign In</h3>
        <form method="POST" action="{{ route('login') }}"> @csrf <input placeholder="Enter Your Email" type="text" id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus /> @error('email') <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
          </span> @enderror 
           
           <div class="d-flex">           
          <input placeholder="Enter Your Password" type="password" id="password" class="form-control @error('password') is-invalid @enderror" name="password" required autocomplete="current-password" /><i class="fa fa-eye" id="togglePassword" style="margin-left: -30px;margin-top: 17px; cursor: pointer;"></i>
          </div>
          ...
          <span class="invalid-feedback" role="alert">
            ...
            <strong>{{ $message }}</strong> 
          </span> @enderror @if (Route::has('password.request')) <a class="btn btn-link" style="color:#000;" href="{{ route('password.request') }}">
            {{ __('Forgot Your Password?') }}
          </a> @endif  
          <button class="text-dark" style="background-color:#E2E2E2; border-radius:10px;">Login</button>
          <p class="message">Want To registered? <a style="rgb(26, 0, 0) !important;" href="{{ route('register') }}">Sign Up</a>
          </p>
        </form>
      </div>
    </div>
